...
vi.mock("./apiClient", () => ({
  getJson: vi.fn().mockRejectedValue(new Error("API unavailable in test")),
}));

const createMockModule = (): EffectModule => ({
  init: vi.fn((_scene: THREE.Scene) => ({})),
  update: vi.fn(),
  dispose: vi.fn(),
});
...
describe("EffectService", () => {
  ...
  describe("loadEffectModule", () => {
    it("효과 모듈 로드 성공", async () => {
      ...
      vi.mocked(EffectLoader.loadEffect).mockResolvedValue(mockModule);
