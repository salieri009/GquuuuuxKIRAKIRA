import React, { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { X, Save, Trash2, Download, Upload } from "lucide-react";
import { useEffectStore } from "../../store/effectStore";
import { useUIStore } from "../../store/uiStore";
import GlassPanel from "../ui/GlassPanel";
import Button from "../ui/Button";
import {
  type Preset,
  loadPresetsFromStorage,
  parsePresetList,
  savePresetsToStorage,
  validatePreset,
} from "../../utils/presetValidation";
import { MAX_PRESET_IMPORT_BYTES } from "../../utils/paramSanitizer";

export default function PresetManager() {
  const { modal, closeModal, showToast } = useUIStore();
  const { selectedEffect, currentParams, updateParams, selectEffect } =
    useEffectStore();
  const [presets, setPresets] = useState<Preset[]>([]);
  const [presetName, setPresetName] = useState("");
  const [pendingDeleteId, setPendingDeleteId] = useState<string | null>(null);

  useEffect(() => {
    setPresets(loadPresetsFromStorage());
  }, []);

  const persistPresets = (next: Preset[]) => {
    savePresetsToStorage(next);
    setPresets(next);
  };

  const savePreset = () => {
    if (!presetName.trim()) {
      showToast("프리셋 이름을 입력해주세요.", "warning");
      return;
    }

    if (!selectedEffect) {
      showToast("효과를 선택해주세요.", "warning");
      return;
    }

    const params = Object.fromEntries(
      Object.entries(currentParams).map(([key, param]) => [key, param.value]),
    );

    const newPreset = validatePreset({
      id: `preset-${Date.now()}`,
      name: presetName.trim(),
      effectId: selectedEffect.id,
      params,
      createdAt: new Date().toISOString(),
    });

    if (!newPreset) {
      showToast("프리셋을 저장할 수 없습니다.", "error");
      return;
    }

    persistPresets([...presets, newPreset]);
    setPresetName("");
    showToast(`프리셋 "${newPreset.name}"이 저장되었습니다.`, "success");
  };

  const deletePreset = (id: string) => {
    if (pendingDeleteId !== id) {
      setPendingDeleteId(id);
      return;
    }

    persistPresets(presets.filter((p) => p.id !== id));
    setPendingDeleteId(null);
    showToast("프리셋이 삭제되었습니다.", "success");
  };

  const applyPreset = (preset: Preset) => {
    selectEffect(preset.effectId);

    setTimeout(() => {
      updateParams(preset.params);
    }, 100);
  };

  const exportPresets = () => {
    const dataStr = JSON.stringify(presets, null, 2);
    const dataBlob = new Blob([dataStr], { type: "application/json" });
    const url = URL.createObjectURL(dataBlob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `kirakira-presets-${new Date().toISOString().split("T")[0]}.json`;
    link.click();
    URL.revokeObjectURL(url);
  };

  const importPresets = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (!file) return;

    if (file.size > MAX_PRESET_IMPORT_BYTES) {
      showToast("프리셋 파일이 너무 큽니다. (최대 256KB)", "warning");
      return;
    }

    const reader = new FileReader();
    reader.onload = (e) => {
      try {
        const imported = parsePresetList(
          JSON.parse(e.target?.result as string),
        );
        if (imported.length === 0) {
          showToast("가져올 수 있는 유효한 프리셋이 없습니다.", "warning");
          return;
        }

        const existingIds = new Set(presets.map((p) => p.id));
        const merged = [
          ...presets,
          ...imported.filter((p) => !existingIds.has(p.id)),
        ];
        persistPresets(merged);
        showToast(`${imported.length}개의 프리셋이 가져와졌습니다.`, "success");
      } catch {
        showToast("프리셋 파일 형식이 올바르지 않습니다.", "error");
      }
    };
    reader.readAsText(file);
  };

  const currentEffectPresets = presets.filter(
    (p) => selectedEffect && p.effectId === selectedEffect.id,
  );

  if (!modal.visible || modal.component !== "presets") {
    return null;
  }

  return (
    <AnimatePresence>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4"
        onClick={closeModal}
      >
        <motion.div
          initial={{ opacity: 0, scale: 0.9, y: 50 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.9, y: 50 }}
          className="w-full max-w-2xl max-h-[90vh] overflow-hidden"
          onClick={(e) => e.stopPropagation()}
        >
          <GlassPanel className="p-lg h-full flex flex-col">
            <div className="flex items-center justify-between mb-lg">
              <h2 className="text-2xl font-bold text-accent">프리셋 관리</h2>
              <Button
                onClick={closeModal}
                size="sm"
                variant="ghost"
                aria-label="닫기"
              >
                <X size={20} />
              </Button>
            </div>

            {selectedEffect && (
              <div className="mb-lg p-md bg-secondary-bg rounded border border-border-primary">
                <h3 className="font-bold text-text-primary mb-sm">
                  새 프리셋 저장
                </h3>
                <div className="flex gap-sm">
                  <input
                    type="text"
                    placeholder="프리셋 이름"
                    value={presetName}
                    onChange={(e) => setPresetName(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        savePreset();
                      }
                    }}
                    className="flex-1 px-sm py-xs bg-primary-bg border border-border-primary rounded-md text-text-primary focus:outline-none focus:ring-1 focus:ring-accent"
                  />
                  <Button onClick={savePreset} leftIcon={<Save size={16} />}>
                    저장
                  </Button>
                </div>
              </div>
            )}

            <div className="flex-1 overflow-y-auto space-y-sm mb-lg">
              {currentEffectPresets.length === 0 ? (
                <div className="text-center py-lg text-text-muted">
                  {selectedEffect
                    ? "이 효과에 대한 프리셋이 없습니다."
                    : "효과를 선택하면 프리셋을 저장할 수 있습니다."}
                </div>
              ) : (
                currentEffectPresets.map((preset) => (
                  <motion.div
                    key={preset.id}
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    className="p-md bg-secondary-bg rounded border border-border-primary flex items-center justify-between gap-sm"
                  >
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-text-primary truncate">
                        {preset.name}
                      </h4>
                      <p className="text-xs text-text-muted mt-xs">
                        {new Date(preset.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                    <div className="flex gap-sm items-center shrink-0">
                      <Button
                        size="sm"
                        variant="primary"
                        onClick={() => applyPreset(preset)}
                      >
                        적용
                      </Button>
                      {pendingDeleteId === preset.id ? (
                        <>
                          <Button
                            size="sm"
                            variant="danger"
                            onClick={() => deletePreset(preset.id)}
                          >
                            삭제
                          </Button>
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => setPendingDeleteId(null)}
                          >
                            취소
                          </Button>
                        </>
                      ) : (
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => deletePreset(preset.id)}
                          aria-label="삭제"
                        >
                          <Trash2 size={16} />
                        </Button>
                      )}
                    </div>
                  </motion.div>
                ))
              )}
            </div>

            <div className="flex gap-sm pt-md border-t border-border-primary">
              <label className="flex-1">
                <input
                  type="file"
                  accept=".json,application/json"
                  onChange={importPresets}
                  className="hidden"
                />
                <span className="inline-flex w-full items-center justify-center gap-2 px-4 py-2 text-sm font-medium rounded-md bg-surface text-text-primary border border-border-primary hover:bg-tertiary-bg cursor-pointer transition-colors duration-150">
                  <Upload size={16} />
                  가져오기
                </span>
              </label>
              <Button
                variant="secondary"
                onClick={exportPresets}
                leftIcon={<Download size={16} />}
                className="flex-1"
              >
                보내기
              </Button>
            </div>
          </GlassPanel>
        </motion.div>
      </motion.div>
    </AnimatePresence>
  );
}
